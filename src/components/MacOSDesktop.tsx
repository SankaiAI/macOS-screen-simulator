...
// Official macOS cursor SVGs from https://mac-cursors.netlify.app/
const MacCursorArrow = () => (
...
// Wallpaper styles based on macOS Sequoia/Sonoma
const wallpaperStyles: Record<WallpaperType, string> = {
    'sequoia': 'bg-gradient-to-br from-orange-600 via-red-500 to-purple-700',
    'sonoma': 'bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400',
    'monterey': 'bg-gradient-to-br from-purple-900 via-blue-800 to-cyan-600',
    'ventura': 'bg-gradient-to-br from-orange-500 via-yellow-400 to-teal-500',
    'big-sur': 'bg-gradient-to-br from-blue-600 via-purple-600 to-pink-500',
    'abstract-dark': 'bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900',
    'abstract-light': 'bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100',
}

interface WebcamBubbleProps {
    showWebcam: boolean
    webcamStream: MediaStream | null
    webcamPos: { x: number; y: number }
    webcamRef: React.RefObject<HTMLDivElement>
    handleWebcamMouseDown: (e: React.MouseEvent) => void
    setIsOverWebcam: (isOver: boolean) => void
    isRecording?: boolean
}
...
const WebcamBubble = ({
    ...
    if (!showWebcam || !webcamStream) return null

    return (
        <div
            ref={webcamRef}
            onMouseDown={handleWebcamMouseDown}
            onMouseEnter={() => setIsOverWebcam(true)}
            onMouseLeave={() => setIsOverWebcam(false)}
            ...
            style={{
                ...
                height: '160px',
                cursor: 'none',
                transition: isRecording ? 'none' : 'left 0.1s ease-out, top 0.1s ease-out, transform 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275)'
            }}
        >
            <div className={`w-full h-full rounded-full border-4 border-white/20 shadow-2xl overflow-hidden bg-black/40 backdrop-blur-sm relative ${isRecording ? '' : 'transition-all hover:scale-105 active:scale-95 duration-200'}`}>
                <video
                    ref={videoRef}
                    autoPlay
                    muted
...
export default function MacOSDesktop({
    ...
    const [isOverSafariWindow, setIsOverSafariWindow] = useState(false)
    const [isOverWebcam, setIsOverWebcam] = useState(false)

    // Auto-hide states for menu bar and dock
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        // Show menu bar when mouse is near top edge (within 8px)
        if (y <= 8) {
            setMenuBarVisible(true)
        } else if (y > 40) {
            // Hide when mouse moves away from menu bar area
            setMenuBarVisible(false)
        }

        // Show dock when mouse is near bottom edge (within 8px)
        if (y >= rect.height - 8) {
            setDockVisible(true)
        } else if (y < rect.height - 80) {
            // Hide when mouse moves away from dock area
            setDockVisible(false)
        }
    }
    ...
    const getCursorIcon = () => {
        switch (cursorStyle) {
            case 'pointer': return <MacCursorPointer />
            default: return <MacCursorArrow />
        }
    }

    // Determine if we should show the custom cursor
    // Hide custom cursor during recording to prevent "two layers" ghosting
    // Show custom cursor if on desktop, or if over webcam even in interaction mode
    const showCustomCursor = isHovering && !isRecording && !isOverResizeHandle && (!isInteracting || !isOverSafariWindow || isOverWebcam)

    // Webcam dragging state
    ...
    useEffect(() => {
        const handleGlobalMouseMove = (e: MouseEvent) => {
            if (!isDraggingWebcam || !desktopRef?.current) return

            const desktopRect = desktopRef.current.getBoundingClientRect()
            let newX = e.clientX - desktopRect.left - dragOffsetRef.current.x
            let newY = e.clientY - desktopRect.top - dragOffsetRef.current.y

            // Simple boundary clamping
            newX = Math.max(10, Math.min(newX, desktopRect.width - 170))
            newY = Math.max(10, Math.min(newY, desktopRect.height - 170))
    ...
    }, [isDraggingWebcam, desktopRef])

    return (
        <div
            ref={desktopRef}
            className={`w-full h-full ${wallpaperStyles[wallpaper]} relative flex flex-col items-center justify-center overflow-hidden transition-colors duration-500`}
        ...
        >
            ...
            {showCustomCursor && (
                <div
                    ...
                    style={{
                        left: cursorPos.x - CURSOR_HOTSPOTS[cursorStyle === 'pointer' ? 'pointer' : 'default'].x,
                        top: cursorPos.y - CURSOR_HOTSPOTS[cursorStyle === 'pointer' ? 'pointer' : 'default'].y,
                        transform: `scale(${cursorSize})`,
                        transformOrigin: `${CURSOR_HOTSPOTS[cursorStyle === 'pointer' ? 'pointer' : 'default'].x}px ${CURSOR_HOTSPOTS[cursorStyle === 'pointer' ? 'pointer' : 'default'].y}px`,
            ...
            {/* Menu Bar - auto-hide with slide animation */}
            {showMenuBar && (
                <div
                    className={`absolute top-0 left-0 right-0 z-20 ${isRecording ? '' : 'transition-transform duration-300 ease-out'} ${isRecording ? '' : '[&_*]:!cursor-none'}`}
                    style={{
                        transform: menuBarVisible ? 'translateY(0)' : 'translateY(-100%)',
                        cursor: isRecording ? 'default' : 'none',
                    }}
                    onMouseEnter={() => setMenuBarVisible(true)}
                    onMouseLeave={(e) => {
                        const rect = e.currentTarget.getBoundingClientRect()
                ...
                >
                    <MenuBar scale={selectedModel?.scale || 1} />
                </div>
            )}

            {/* Top edge trigger zone for menu bar */}
            {showMenuBar && !menuBarVisible && (
                <div
                    className="absolute top-0 left-0 right-0 h-2 z-10"
                    style={{ cursor: isRecording ? 'default' : 'none' }}
                    onMouseEnter={() => setMenuBarVisible(true)}
                />
            )}

            <div
                className={(isOverResizeHandle || isRecording || (isInteracting && isOverSafariWindow && !isOverWebcam)) ? '' : '[&_*]:!cursor-none'}
                style={{ cursor: (isOverResizeHandle || isRecording || (isInteracting && isOverSafariWindow && !isOverWebcam)) ? 'default' : 'none' }}
            >
                <SafariWindow
                    url={url}
                    ...
                    onResizeHover={setIsOverResizeHandle}
                    isInteracting={isInteracting}
                    setIsInteracting={setIsInteracting}
                    onInteractionHover={setIsOverSafariWindow}
                />
            </div>

            {/* Dock - auto-hide with slide animation */}
            {showDock && (
                <div
                    className={`absolute bottom-0 left-0 right-0 ${isRecording ? '' : 'transition-transform duration-300 ease-out'} ${isRecording ? '' : '[&_*]:!cursor-none'}`}
                    style={{
                ...
                >
                    <Dock itemSize={selectedModel?.dockItemSize || 48} />
                </div>
            )}
            ...
            <WebcamBubble
                ...
                webcamPos={webcamPos}
                webcamRef={webcamRef}
                handleWebcamMouseDown={handleWebcamMouseDown}
                setIsOverWebcam={setIsOverWebcam}
                isRecording={isRecording}
            />

            {/* Bottom edge trigger zone for dock */}
